def load_climate_data(data_in, model, scalars, site, folder_in, proj_pre_raw, proj_temp_raw, last_hist_date):
    ...
    data_merged = data_in.merge(climate_data[['date', 'precip_in_proj', 'mean_temp_c_proj']], on='date', how='outer') ## join climate data and historical data 
    ...
    data_out['mean_temp_c'].fillna(data_out['mean_temp_c_proj'], inplace=True)
    data_out.drop(['mean_temp_c_proj', 'precip_in_proj'], axis=1, inplace=True)
